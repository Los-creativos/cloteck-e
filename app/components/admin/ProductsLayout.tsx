...
export default function ProductsLayout({ page }: { page: string }) {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    await findAllProducts(pageInt, ITEMS_PAGE)
      .then((res) => {
        setProducts(res);
      })
      await findAllProducts(2, ITEMS_PAGE).then((res) => {
  ...
  return (
    <div>
      {loading ? (
        <>
          <div className="text-xl text-center justify-center">Loading...</div>
        </>
      ) : (
        <>
                  ...
                  <ProductDisplay
                    ...
                    Edit={
                      <LinkButton
                        ...
                        href={`/admin/product/edit/${encodeURIComponent(
                    ...
                    Remove={
                      <Button
                        className="bg-transparent"
                        onClick={() => handleOnClick(product.product_id)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
          ...
          <div className="flex justify-center items-center">
            ...
            {nextPageProducts.length > 0 && products.length === ITEMS_PAGE && (
              <LinkButton
                className="bg-transparent"
